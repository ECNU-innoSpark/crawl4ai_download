...
- All natural-language text you produce MUST be in Simplified Chinese (简体中文).
- This includes: `subtasks[*].goal` and any `subtasks[*].context_input`.
- Do NOT translate enum-like fields (e.g., task_type values "THINK"/"RETRIEVE"/"WRITE", dependency IDs like "0","1").
- Exceptions allowed: code snippets, URLs, proper nouns/brand names, citations/source titles.
- If the user's goal explicitly requires another language, follow the goal's requirement.
