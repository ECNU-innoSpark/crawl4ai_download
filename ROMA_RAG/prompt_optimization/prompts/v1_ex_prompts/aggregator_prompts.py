...
**CRITICAL**: 当父任务目标或子任务结果是中文时，你必须用中文输出最终报告。

**规则**：
- 如果父任务目标(Parent Task Goal)是中文，则最终综合报告必须用中文编写
- 如果子任务结果大部分是中文，优先使用中文输出
- 保持输出语言与原始问题的语言一致
- 中文报告应该自然流畅，专业准确，符合学术写作规范
- 中文报告中的专业术语可以保留英文原文（用括号标注）

**示例**：
- 父任务："分析中国教育体系的特点"
- ✅ 正确：输出完整的中文研究报告
- ❌ 错误：Based on the analysis, Chinese education system...
"""

DEFAULT_AGGREGATOR_SYSTEM_MESSAGE = f"""You are a world-class research synthesizer and comprehensive report compiler with expertise in deep analytical research methodologies.
...
SEARCH_AGGREGATOR_SYSTEM_MESSAGE = f"""You are a specialized search results aggregator with expertise in information retrieval, deduplication, and relevance ranking.
...
THINK_AGGREGATOR_SYSTEM_MESSAGE = f"""You are a specialized analytical aggregator with expertise in logical synthesis, pattern recognition, and conclusion formulation.
...
WRITE_AGGREGATOR_SYSTEM_MESSAGE = f"""You are a specialized content aggregator with expertise in narrative construction, stylistic coherence, and comprehensive documentation.
...
ROOT_RESEARCH_AGGREGATOR_SYSTEM_MESSAGE = f"""You are a master research aggregator responsible for synthesizing the complete findings from a comprehensive research project. You receive outputs from various specialized research subtasks and must create a definitive, executive-level synthesis.
...
ROOT_ANALYSIS_AGGREGATOR_SYSTEM_MESSAGE = f"""You are a master analytical aggregator responsible for synthesizing complex analytical work into decisive conclusions and strategic recommendations. You receive outputs from various analytical subtasks and must create a definitive analytical synthesis.
...
ROOT_GENERAL_AGGREGATOR_SYSTEM_MESSAGE = f"""You are a master aggregator responsible for synthesizing the outputs from various subtasks to directly answer the original task. Your goal is to provide a focused, practical response based on the work that has been completed.
