...
class E2BToolkit(BaseToolkit):
    ...
    def _initialize_tools(self) -> None:
        ...
        # Set E2B_TEMPLATE_ID=roma-sandbox-sc for production
        self.template = (
    ...
    async def _create_sandbox(self) -> object:
        ...
        if enable_s3:
            ...
            env_vars = {
                ...
                # Template/runtime scripts expect S3_BUCKET_NAME
    ...
    def _is_transient_startup_error(self, e: Exception) -> bool:
        """
        Detect transient E2B startup errors.

        E2B Code Interpreter occasionally returns 502 "port is not open" during
        cold starts while the internal service (usually port 49999) is still
        booting. Retrying for a short period typically resolves it.
        """
        msg = str(e)
        return ("port is not open" in msg) or ('"port":49999' in msg and '"code":502' in msg)
    ...
    async def run_python_code(self, code: str) -> str:
        ...
        try:
            # Cold-start resilience: the sandbox may be running before the
            # code-interpreter service is ready (common 502 "port not open").
            # Some environments (S3 mount + cold image pull) can take >60s
            # before the internal code server starts listening.
            startup_max_wait = float(self.config.get("startup_max_wait_seconds", 180))
            delay = 1.0
            start_ts = time.time()
            ...
            while True:
                try:
                    ...
                    if not self._is_transient_startup_error(e):
                        raise
                    if (time.time() - start_ts) >= startup_max_wait:
                        raise
                    self.log_warning(
                        "Sandbox not ready yet (code server port not open). "
            ...
            while True:
                try:
                    ...
                    if (time.time() - start_ts) >= startup_max_wait:
                        raise
                    self.log_warning(
                        "Sandbox not ready yet (command server port not open). "
                        f"Retrying in {delay:.1f}s..."
                    )
                    await asyncio.sleep(delay)
                    delay = min(delay * 2, 8.0)
