...
class MCPToolkit(BaseToolkit):
    ...
    async def _initialize_with_fastmcp(self) -> None:
        ...
        # FastMCP Client is the session - use self._context for tools
        # This ensures tools access the persistent connection, not a local variable

        # Step 3: List tools from server
        tools = await self._context.list_tools()

        # Step 4: Convert each tool to DSPy Tool with full schema extraction
        for tool in tools:
            # Convert MCP tool to DSPy Tool (preserves schema metadata)
            # CRITICAL: Pass self._context (persistent) not local variable
            dspy_tool = _convert_mcp_tool_to_dspy(client=self._context, tool=tool)
    ...
    def _register_all_tools(self) -> None:
        """Override BaseToolkit's automatic tool registration.

        MCPToolkit tools are dynamically loaded from MCP server and stored in
        self._mcp_tools during _async_initialize(). They are not class methods,
        so we skip the automatic method-based registration that BaseToolkit does.

        Tools are already wrapped with metrics tracking in _initialize_with_fastmcp().
        """
        # Skip automatic registration - MCP tools are dynamically loaded
        # and already stored in self._mcp_tools with proper wrapping
        pass
    ...
    def get_enabled_tools(self) -> Dict[str, Any]:
        ...
        # Apply include filter (with safe attribute access)
        # 防御性检查：如果属性不存在，使用默认值
        include_tools = getattr(self, 'include_tools', None)
        if include_tools:
            enabled = set(include_tools) & available
        else:
            enabled = available

        # Apply exclude filter (with safe attribute access)
