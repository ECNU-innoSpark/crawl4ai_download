...
class ExecutionContext:
    ...
    def __init__(self, execution_id: str, file_storage: "FileStorage"):
        ...
        # Collected sources buffer for provenance tracking
        # This allows tools to register sources out-of-band from their text output
    ...
    @classmethod
    def add_sources(cls, sources: List[str]) -> None:
        """
        Add sources to the current execution context.
        
        Args:
            sources: List of source URLs to add
        """
        ctx = cls.get()
        if ctx and sources:
            ctx.collected_sources.extend(sources)

    @classmethod
    def get_and_clear_sources(cls) -> List[str]:
        """
        Get collected sources and clear the buffer.
        
        Returns:
            List of unique collected sources
        """
        ctx = cls.get()
        if not ctx:
            return []
        ...
        sources = []
        seen = set()
        for s in ctx.collected_sources:
            if s not in seen:
                sources.append(s)
                seen.add(s)
        
        # Clear buffer
        ctx.collected_sources.clear()
        return sources
