...
class ModuleRuntime:
    ...
    async def _execute_agent_with_tracing(
        ...
        *,
        ...
        """
        # Agent routing is normally task-type aware (AgentMappingConfig).
        #
        # However, the root task (depth=0, parent_id=None) is created from a raw string
        # and TaskNode.task_type defaults to THINK. That default is not semantic "root"
        # intent and can accidentally route the *initial* planning step into the THINK
        # planner variant (e.g., DR THINK planner), which is meant for second-level
        # decomposition of THINK subtasks.
        #
        # For root-level orchestration calls, force the default agent (task_type=None).
        # Rationale:
        # - Root TaskNode is created from a raw string and TaskNode.task_type defaults to THINK.
        # - That default is not semantic "root" intent and can accidentally route the initial
        #   PLANNER/AGGREGATOR into task-specific variants (e.g., DR THINK planner/aggregator),
        #   which are designed for second-level handling of typed subtasks.
    ...
    async def execute_async(self, task: TaskNode, dag: TaskDAG) -> TaskNode:
        ...
        def process_result(
            ...
            # Capture ReAct trajectory for deep research aggregation
            ...
            for node in subgraph.get_all_tasks(include_subgraphs=False):
                ...
                collected.append(
                    SubTask(
                        ...
                        sources=sources,  # Pass sources to aggregator
                    )
                )
        return collected
