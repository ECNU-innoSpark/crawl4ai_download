    ...
    def on_mount(self) -> None:
        ...
        async def load_data_background() -> None:
            ...
            try:
                ...
                # Even if loading fails, mark as loaded to show error
                logger.error(f"Failed to load data in background: {exc}", exc_info=True)
                # Show error message on welcome screen
                message_widget = welcome_screen.query_one("#welcome-message", Static)
                error_msg = str(exc)[:100]  # Truncate long errors
                message_widget.update(f"[red]✗ Load failed: {error_msg}[/red]")
                # Still mark as loaded so user can dismiss the screen
                welcome_screen.mark_data_loaded()
                # Re-raise to allow error handling in _load_from_api
                raise
