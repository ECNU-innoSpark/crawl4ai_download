...
class WelcomeScreen(ModalScreen[None]):
    ...
    def mark_data_loaded(self) -> None:
        ...
        # Only show success message if not already showing an error
        # Try to get current message content safely
        try:
            # In Textual, Static content can be accessed via renderable attribute
            # But this may not be available in all versions, so we use hasattr check
            if hasattr(message_widget, 'renderable'):
                current_message = getattr(message_widget, 'renderable', "")
            else:
                # If renderable is not available, just update without checking
                current_message = ""
        except (AttributeError, Exception):
            # If any error occurs accessing renderable, just proceed with update
            current_message = ""
        
        # Check if current message contains error (red markup)
        # Only skip success message if we detected an error message
        if not (isinstance(current_message, str) and "[red]" in str(current_message)):
