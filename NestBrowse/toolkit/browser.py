...
# ViNest: 从 MCP 返回的文本中解析截图块
SCREENSHOT_MARKER_START = "[SCREENSHOT_BASE64]"
SCREENSHOT_MARKER_END = "[/SCREENSHOT_BASE64]"


def _extract_screenshot_from_response(raw_response_text: str) -> tuple[str, Optional[str]]:
    """
    从 MCP 返回的文本中分离正文与 Base64 截图。
    若包含 [SCREENSHOT_BASE64]...[/SCREENSHOT_BASE64] 块则返回 (raw_text, screenshot_base64)，否则 (raw_response_text, None)。
    """
    if not raw_response_text or SCREENSHOT_MARKER_START not in raw_response_text or SCREENSHOT_MARKER_END not in raw_response_text:
        return raw_response_text, None
    parts = raw_response_text.split(SCREENSHOT_MARKER_START, 1)
    raw_text = parts[0].strip()
    rest = parts[1].split(SCREENSHOT_MARKER_END, 1)
    screenshot_base64 = rest[0].strip() if rest[0].strip() else None
    return raw_text, screenshot_base64
...
class Visit:
    ...
    async def call(self, params, **kwargs):
        ...
        try:
            ...
            async with lock:
                # ViNest: 请求截图以支持视觉内循环
                response = await client.call_tool('browser_navigate', {'url': url, 'include_screenshot': True})
        ...
        if not response.isError:
            raw_text, screenshot_base64 = _extract_screenshot_from_response(raw_response_text)
            try:
                response_text, record = await process_response(raw_text, goal, os.getenv("SUMMARY_MODEL_NAME", os.getenv("MODEL_NAME")), tokenizer, sem, screenshot=screenshot_base64)
            except Exception as e:
                print(f"[visit] process_response error: {e}")
...
class Click:
    ...
    async def call(self, params, **kwargs):
        ...
        try:
            ...
            async with lock:
                # ViNest: 请求截图以支持视觉内循环
                response = await client.call_tool('browser_click', {'ref': ref, 'element': '', 'include_screenshot': True})
            raw_response_text = response.content[0].text
        except Exception:
        ...
        if not response.isError:
            ...
            try:
                ...
                print(f"[click] process_response error: {e}")
                response_text = "Evidence in page: \n" + "The provided webpage content could not be accessed. Please check the input format." + "\n\n" + "Summary: \n" + "The webpage content could not be processed, and therefore, no information is available."
                record = []
