...
**Final Output Format using JSON format has "rational", "evidence", "summary", "refinement" fields (refinement can be null).**
""".strip()
...
Your task: given webpage content and a user goal, extract and organize the useful information according to the following schema: {"rational": "string", "evidence": "string", "summary": "string", "refinement": "string | null"}. 
...
4) refinement (Adaptive Goal Refinement): If the user goal is unclear, impossible to satisfy on this page, or you find a critical clue that suggests a better direction (e.g., a link, a hint, or a correction), output a short suggestion in this field; otherwise use null. This will be shown to the outer-loop agent as urgent advice.

Formatting requirements: Output only one valid JSON object wrapped inside <useful_info> and </useful_info> tags: use double quotes (") for all keys and string values, no trailing commas, and the top-level structure must be exactly: {"rational": "...", "evidence": "...", "summary": "...", "refinement": "..." or null}.
""".strip()
